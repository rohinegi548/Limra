...
public class TeamSetup_Ticket_82 {
	...
	@Test //Check the Checkboxes appears correctly
		  //And the Checkboxes text matches Financial Model sheet CELLS B39 through B42
	public void DoesChkboxTxtMatchesOfMANAGER_SELECTION_Grid() throws InterruptedException, AWTException{
		...
		List<WebElement> SlctnPROCESSStepChkTxt=driver.findElements(By.xpath("//*[contains(@class,'column managerColumn')]/div[contains(@class,'group')]//span[contains(@class,'label')]"));
		int flag=0;
		
		System.out.println("Actual checkbox text==Expected text");
		for(int j=1;j<=SlctnPROCESSStepChkTxt.size();j++){
			
			if(SlctnPROCESSStepChkTxt.get(j-1).getText().equals(SlctnSteps[j-1])){
				
				System.out.println(SlctnPROCESSStepChkTxt.get(j-1).getText()+"=="+SlctnSteps[j-1]);
				flag=flag+1;
			}
				
		}
		
		Assert.assertEquals(flag, 4, "Fieldset legend text doesn't match Financial Model sheet and "
				+ "And the Checkboxes text doesn't match Financial Model sheet for MANAGER_SELECTION grid\n:Failing Test!");
		
		
		System.out.println("Fieldset legend text matches Financial Model sheet and "
				+ "And the Checkboxes text matches Financial Model sheet for MANAGER_SELECTION grid\n:Passing Test!");
	...
	public void CanUserAbleToToggleCheckboxesOfMANAGER_SELECTION_Grid() throws InterruptedException, AWTException{
		...
		for(int j=1;j<=SlctnPROCESSStepChkTxt.size();j++){
			...
			SlctnPROCESSStepChkTxt.get(j-1).click(); Thread.sleep(1000);
			flag=flag+1;
				
		}
		
		Assert.assertEquals(flag, 4, "Checkboxes doesn't behave correctly\n:Failing Test!");
		
		
		System.out.println("Checkboxes behave correctly, able to select, deselect and "
				+ "A tick is displayed as per the art direction for MANAGER_SELECTION grid\n:Passing Test!");
